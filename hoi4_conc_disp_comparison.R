...
ic_c = vector("numeric", 500)
ic_d = vector("numeric", 500)
for(i in 1:500){
...
plot_data = data.frame(day=rep(1:500,times=2),ic=c(ic_cum_c,ic_cum_d),type=rep(c('Concentrated','Dispersed'),each=500))
...
ggplot(plot_data) + geom_line(aes(x=day,y=ic,colour=type))+
  geom_label(aes(x=day,y=ic,label = format(round(ic,2),big.mark=",",scientific=FALSE)), 
             data = . %>% filter(row_number() == 500),nudge_x=-10, nudge_y=100) + 
